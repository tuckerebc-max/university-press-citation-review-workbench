...
from upress_workbench.artifacts import atomic_write_bytes
from upress_workbench.citations import build_inventory
from upress_workbench.ingest import extract_chapter
from upress_workbench.model_schemas import rci_schema, sei_schema
from upress_workbench.packets import build_chapter_packet
from upress_workbench.provider import CallBudget, CodexCliClient, normalize_model
...
class StubClient:
    model = "default"
    provider_name = "Codex CLI test stub"
...
class WorkbenchCoreTests(unittest.TestCase):
    def test_atomic_write_uses_short_temporary_leaf(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            destination = Path(temporary) / ("long-review-name-" * 8 + ".docx")
            with patch("upress_workbench.artifacts.tempfile.mkstemp", wraps=tempfile.mkstemp) as mocked:
                atomic_write_bytes(destination, b"review")
            self.assertEqual(mocked.call_args.kwargs["prefix"], ".tmp-")
            self.assertEqual(destination.read_bytes(), b"review")

    def test_codex_model_identifier_is_bounded(self) -> None:
        self.assertEqual(normalize_model(""), "default")
        self.assertEqual(normalize_model("gpt-5.6-sol"), "gpt-5.6-sol")
        for unsafe in ["--dangerously-bypass-approvals-and-sandbox", "model\n--help", "model value", "../model"]:
            with self.assertRaises(SafetyError):
                normalize_model(unsafe)

    def test_codex_client_disables_tools_and_validates_output(self) -> None:
        schema = {
            "type": "object",
        ...
        def fake_run(command, **kwargs):
            ...
            output_path.write_text('{"status":"ready"}', encoding="utf-8")
            events = '{"type":"turn.completed","usage":{"input_tokens":11,"output_tokens":3}}\n'
            return subprocess.CompletedProcess(command, 0, stdout=events, stderr="")

        client = CodexCliClient("gpt-5.6-sol", CallBudget(2, 10_000), executable=sys.executable)
        with patch("upress_workbench.provider.subprocess.run", side_effect=fake_run) as mocked:
            result, usage = client.complete_json("system", '{"untrusted":"data"}', "test_schema", schema)
        command = mocked.call_args.args[0]
        self.assertEqual(result, {"status": "ready"})
        self.assertEqual(usage["provider"], "Codex CLI")
        ...
        self.assertIn("USER DATA", mocked.call_args.kwargs["input"])
    ...
    def test_deep_windows_output_path_is_rejected_before_model_work(self) -> None:
        if os.name != "nt":
            self.skipTest("Windows path budget applies only on Windows")
        deep_parent = Path("C:/") / ("deep-output-folder-" * 10)
        with self.assertRaises(SafetyError):
            validate_output_path_budget(deep_parent, ["a-long-chapter-name.docx"])
    ...
    def test_recovery_only_interrupts_orphaned_processes(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            record = {
                ...
                "model": "default", "crossref_enabled": 0, "concurrency": 1, "run_mode": "baseline",
  ...
  <Relationship Id='rId1' Type='http://schemas.openxmlformats.org/officeDocument/2006/relationships/attachedTemplate' Target='file:///C:/hostile.dotm' TargetMode='External'/>
    ...
    def test_offline_review_and_author_packet(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            validate(rci_work["model_result"], rci_schema(rci_policy.rule_ids))
            validate(sei_work["model_result"], sei_schema(sei_policy.rule_ids))

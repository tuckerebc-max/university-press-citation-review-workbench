...
from .provider import inspect_codex_cli
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _error(self, status: int, message: object) -> None:
        self._json({"error": scrub_message(message)}, status)
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/health":
            ...
            codex = inspect_codex_cli()
            self._json({
                "app": config.APP_NAME,
                "version": config.APP_VERSION,
                "provider": config.PROVIDER_NAME,
                "provider_ready": bool(codex.get("available") and codex.get("authenticated")),
                "codex": codex,
                "model": config.DEFAULT_BATCH_MODEL,
                "skills": skills,
            })
            return
        if path == "/api/runs":

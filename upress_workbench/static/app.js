...
function formPayload() {
  ...
  return {
    ...
    model: String(data.get("model") || "default").trim(),
...
async function checkHealth() {
  ...
  try {
    ...
    const ready = value.provider_ready && skillsReady;
    const version = value.codex && value.codex.version ? value.codex.version : "Codex CLI";
    node.textContent = ready ? `${version} · signed in` : "Codex setup needs attention";

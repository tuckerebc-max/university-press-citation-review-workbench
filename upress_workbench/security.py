...
def validate_docx_archive(path: Path | BinaryIO) -> None:
    try:
...
def safe_slug(value: str, fallback: str = "chapter", max_length: int = 80) -> str:
    value = re.sub(r"[^A-Za-z0-9._-]+", "-", value).strip("-._")
    return (value[:max_length] or fallback).lower()


def validate_output_path_budget(output_parent: Path, relative_paths: list[str]) -> None:
    """Fail before model calls when Explorer-friendly output paths would be too long."""
    if os.name != "nt":
        return
    run_leaf = f"{config.RUN_FOLDER_PREFIX}-{'0' * 16}-{'0' * 8}"
    for index, relative_path in enumerate(relative_paths, 1):
        chapter_id = f"CH{index:03d}"
        slug = safe_slug(Path(relative_path).stem, max_length=config.MAX_OUTPUT_SLUG_CHARS)
        candidate = output_parent / run_leaf / "chapters" / f"{chapter_id}-{slug}" / "author-review" / f"{slug}__AUTHOR_REVIEW.docx"
        if len(str(candidate)) > config.MAX_WINDOWS_OUTPUT_PATH_CHARS:
            raise SafetyError(
                "The selected output folder is too deep for reliable Windows and Word paths. "
                "Choose a shorter output parent, such as C:\\Press\\Reviews."
            )

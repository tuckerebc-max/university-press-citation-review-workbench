...
from .provider import JsonCompletionClient
...
def run_sei(
    ...
    result = {
        ...
        "ai_events": [{
            ...
            "tool_name": client.provider_name,
...
def author_items(rci_result: dict[str, Any], rci_workbench: dict[str, Any], sei_result: dict[str, Any], sei_workbench: dict[str, Any]) -> list[dict[str, Any]]:
    rci_raw = {
        finding["finding_id"]: model_item
        for finding, model_item in zip(rci_result["findings"], rci_workbench["model_result"]["findings"], strict=True)
    }
    sei_raw = {
        finding["finding_id"]: model_item
        for finding, model_item in zip(sei_result["findings"], sei_workbench["model_result"]["findings"], strict=True)
    }
    items = []

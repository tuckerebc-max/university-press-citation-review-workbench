...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--model", default=config.DEFAULT_BATCH_MODEL, help="Codex model identifier. Omit or use 'default' for the Codex product default.")
    parser.add_argument("--concurrency", type=int, choices=range(1, config.MAX_WORKERS + 1), default=1)
    parser.add_argument("--crossref", action=argparse.BooleanOptionalAction, default=False, dest="crossref_enabled")
    parser.add_argument("--approve-codex", action="store_true", dest="provider_approved", help="Required confirmation that the signed-in Codex account is approved for every chapter in this manifest.")
    args = parser.parse_args(argv)
    if not args.provider_approved:
        parser.error("--approve-codex is required before manuscript text can be sent through Codex")

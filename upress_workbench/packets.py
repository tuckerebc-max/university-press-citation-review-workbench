...
def build_chapter_packet(
    ...
    slug = safe_slug(Path(chapter.relative_path).stem, max_length=config.MAX_OUTPUT_SLUG_CHARS)
    folder_name = f"{chapter.chapter_id}-{slug}"
    ...
    staging = chapters_root / f".staging-{chapter.chapter_id}-{uuid.uuid4().hex[:8]}"

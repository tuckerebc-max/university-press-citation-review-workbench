...
def atomic_write_bytes(path: Path, payload: bytes) -> None:
    ...
    # Keep the temporary leaf short so a valid Windows output path does not
    # cross MAX_PATH only because the destination filename was repeated.
    handle, temporary = tempfile.mkstemp(prefix=".tmp-", suffix="", dir=path.parent)
    try:

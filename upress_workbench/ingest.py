...
from .security import SafetyError, SourceFile, read_approved_bytes, sha256_file, validate_docx_archive
...
def _extract_note_part(payload: bytes, member: str, context_type: str) -> list[Block]:
    blocks: list[Block] = []
...
def _extract_docx(source: SourceFile, chapter_id: str) -> Chapter:
    payload = read_approved_bytes(source.path, source.sha256)
    validate_docx_archive(io.BytesIO(payload))
    ...
    footnotes = _extract_note_part(payload, "word/footnotes.xml", "footnote")
    endnotes = _extract_note_part(payload, "word/endnotes.xml", "endnote")
...
def _extract_text(source: SourceFile, chapter_id: str) -> Chapter:
    try:
        text = read_approved_bytes(source.path, source.sha256).decode("utf-8", errors="strict")
